# Payroll Management System — FastAPI Backend & Admin Dashboard

A robust, fault-tolerant Fintech payroll system featuring an asynchronous FastAPI Python backend, a PostgreSQL/SQLite database engine (SQLAlchemy 2.0 async), and a clean, single-process Admin Dashboard.

---

## Key Highlights & System Design

### Asynchronous Fault-Tolerant Payroll Distribution
Payroll calculations and distributions are processed using a **per-employee transaction isolation** model rather than wrapping the entire batch in a single database transaction. This guarantees resilient execution:
...
2. **Resilience to Failures**: If processing for a single employee fails (due to database locks, invalid profile configurations, or calculation errors), their transaction is rolled back individually and logged as `FAILED` (generating a `PAYROLL_FAILED` notification). All successful employees remain paid, and their status stays `SUCCESS` in the database.
...
### Single-Page Admin Dashboard Frontend
A built-in admin dashboard (`static/index.html`) is served directly from the root URL (`/`) using FastAPI's `HTMLResponse`. It runs on the same port and server process, providing:
- **Login screen**: Forces secure JWT authentication (in-memory token storage) and blocks non-ADMIN roles.
- **Trigger panel**: Allows selecting a month and year to trigger payroll.
- **Live Status table**: Polls status updates every 2 seconds, resolving employee details dynamically using a client-side mapping cache (requiring zero API changes). Shows distinct colored badges for `SUCCESS`, `PENDING`, and `FAILED` states.
- **Retry Panel**: Displays a "Retry Failed Payouts" button if a run is `PARTIAL`.
- **Payslip Modal**: Displays masked bank accounts (`******7890`), custom payslip numbers, and itemized component breakdowns.
...
6. **Start the Development Server**:
   ...
   ```
   Now visit **[http://127.0.0.1:8000/](http://127.0.0.1:8000/)** in your browser to access the live Admin Dashboard! Log in with:
